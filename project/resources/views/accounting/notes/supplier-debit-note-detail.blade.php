...
@section('title','Debit Note Detail | Accounting')
...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Debit Note Detail</li>
      </ol>
  </div>
</div>
...
<div class="row mb-0">
  ...
  <div class="row mb-0 headings-color mb-1">
    <div class="col-lg-8 col-md-6">
      <h3 class="maintitle text-uppercase fontbold">Debit Note <span id="po-no-id">{{$getPurchaseOrder->p_o_statuses->parent->prefix . $getPurchaseOrder->ref_id}}</span></h3>
    </div>
  ...
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    <li class="fontbold" style="width: 180px;">Debit Note Date: <b style="color: red;">*</b></li>
...
<div class="modal" id="file-modal">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Debit Note Files</h4>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
...
$('#add_notes_modal').on('hidden.bs.modal', function () {
  ...
  $(document).on("change focusout", "#target_receive_date, #invoice_date, #payment_due_date", function(e) {
    ...
    if (attr_name == 'invoice_date') {
      if ($(this).val() == '') {
        $(this).prev().html("Credit NoteDate Here");
        $(this).addClass('d-none');
        $(this).removeClass('active');
        $(this).prev().removeClass('d-none');
        return false;
  ...
  $(document).on('click', '.confirm-po-btn', function(e) {
    ...
    swal({
        title: "Are you sure!!!",
        text: "You want to confirm this Debit Note?",
        type: "warning",
        showCancelButton: true,
        confirmButtonColor: "#DD6B55",
        ...
        if (isConfirm) {
          ...
          $.ajax({
            ...
            success: function(response) {
              ...
              {
                toastr.success('Success!', 'Debit Note Confirmed Successfully.', {
                  "positionClass": "toast-bottom-right"
                });
                setTimeout(function() {
                    window.location.href = "{{ route('debit-notes-dashboard')}}?type=supplier";
                }, 800);
              }
