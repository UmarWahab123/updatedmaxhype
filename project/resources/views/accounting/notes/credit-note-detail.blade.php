...
<div class="row mb-3">
  <div class="col-md-8 title-col">
    <h5 class="maintitle text-uppercase fontbold mb-0">Credit Note # <span class="c-ref-id">{{$order->status_prefix}}{{$order->ref_prefix}}{{$order->ref_id }}</span></h5>
  </div>
    ...
    <div class="row">
      ...
      <div>
       ...
       <!--  <div class="form-group">
        ...
        <div class="form-group">
          <label class="mb-1 font-weight-bold">Created Date:</label> <span class="dblclk-edit memo-date" data-type="date_picker">{{ $order->created_at->format('d/m/Y H:i:s') }}</span>
        </div>
      ...
      <div class="col-lg-4 col-md-6">
        ...
        @endif
       @if(Auth::user()->role_id == 1 || Auth::user()->role_id == 7 || Auth::user()->role_id == 4 || Auth::user()->role_id == 11)
  ...
  <div class="customer-addresses">
      ...
      <li><b>Tax ID:</b> @if($billing_address->tax_id !== null) {{ $billing_address->tax_id }} @endif</li>
        ...
        @endif
         ...
         @php
          $credit_note_date = $order->credit_note_date != null ? Carbon::parse($order->credit_note_date)->format('d/m/Y') : "";
        @endphp
         <ul class="d-flex list-unstyled mb-0">
          <li class="pt-2 fontbold" style="width: 180px;">Credit Note Date: </li>
          <span class="pl-4 pt-2 inputDoubleClick" id="credit_note_date_span" data-fieldvalue="{{$credit_note_date}}">
            @if($order->credit_note_date != null)
            {{Carbon::parse($order->credit_note_date)->format('d/m/Y')}}
            @else
            <p style="font-weight: 100;">Credit Note Date</p>
            @endif
          </span>
          <input type="text" class="ml-4 mt-2 d-none credit_note_date" name="credit_note_date" id="credit_note_date" value="{{$credit_note_date}}">
        </ul>
      ...
      <table class="table entriestable table-bordered table-quotation-product text-center">
        <thead>
          <tr>
            ...
            <th class="inv-head"># {{$global_terminologies['pieces']}}
            ...
            <th class="inv-head">*{{$global_terminologies['reference_price']}}
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="row">
            ...
            <div class="form-group col-md-6">
              <label for="business_state">District:<b style="color:red;">*</b></label>
...
$(".state-tags").select2();
  ...
  $("#delivery_request_date, #payment_due_date, #target_ship_date, #credit_note_date").datepicker({
    format: "dd/mm/yyyy",
    autoHide: true,
  });

  $(document).on("change focusout","#delivery_request_date, #payment_due_date, #target_ship_date, #credit_note_date",function(e) {
    var quotation_id = "{{ $id }}";
  ...
  $(document).on('click', '#addInquiryProductbtn', function(e){
    ...
    if(paid == 31)
    {
      toastr.warning('Alert!', 'Cannot Add Inquiry Product In Paid Credit Notes',{"positionClass": "toast-bottom-right"});
      return false;
    }
      e.preventDefault();
      $.ajaxSetup({
...
$(document).ready(function(){
  ...
  $(document).on('click','.invoice-btn',function (e) {
    ...
    else
    {
      ...
      $.ajax({
        url: "{{ route('complete-credit-note') }}",
        method: 'post',
        ...
        success: function(result)
        {
          ...
          if(result.success == true)
          {
            toastr.success('Success!','Credit Note Completed Successfully!',{"positionClass": "toast-bottom-right"});
            // setTimeout(function(){
