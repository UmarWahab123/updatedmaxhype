...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Margin Report by Customer</li>
      </ol>
  </div>
</div>
...
@if($customer_type_filter == 'yes')
<div class="row mb-0">
  <div class="col-lg-12 col-md-12 title-col">
    <div class="col-md-3">
      <label>Secondary Filter</label>
      <div class="form-group mb-0">
        <input style="background-color:white;" type="text" class="form-control selecting-cust-cat" name="selecting-cust-cat" disabled readonly value="{{$customer_category->title}}" data-id="{{$customer_category->id}}">
      </div>
    </div>
  </div>
</div>
    ...
    <div class="col-md-3">
      <label>Sale Person</label>
        <select class="font-weight-bold form-control-lg form-control state-tags customer_selected_select" disabled="disabled">
          ...
          <option value="{{$sale->id}}" {{$sale_person_selected == $sale->id ? 'selected' : ''}}> {{$sale->name}} </option>
          @endforeach
    ...
    <div class="entriesbg bg-white custompadding customborder">
      <table class="table entriestable table-bordered text-center product-sales-report">
        ...
        <thead>
          <tr>
            <th>Customer Ref #
              <span class="arrow_up sorting_filter_table" data-order="2" data-column_name="customer_ref_no">
                <img src="{{url('public/svg/up.svg')}}" alt="up" style="width:10px; height:10px; cursor: pointer;">
              </span>
              <span class="arrow_down sorting_filter_table" data-order="1" data-column_name="customer_ref_no">
  ...
  $(function(e){
    ...
    //   processing: '<i class="fa fa-spinner fa-spin fa-3x fa-fw" style="color:#13436c;"></i><span class="sr-only">Loading...</span> '},
      ...
      ajax: {
        ...
        url:"{!! route('get-margin-report-5') !!}",
        data: function(data) {data.from_date = $('#from_date').val(),
         data.to_date = $('#to_date').val(), data.product_id = $('.product_id option:selected').val(),data.selecting_cust_cat = $('.selecting-cust-cat').data('id'),
         data.sortbyparam = column_name,
         data.sortbyvalue = order,
         data.sale_person_selected = $('.customer_selected_select option:selected').val()
          },
        method: "get",
      },
      columns: [
      ...
      //   $( api.column( 2 ).footer() ).html(total_vat_out);
      //   $( api.column( 3 ).footer() ).html(total_sales);
      //   $( api.column( 4 ).footer() ).html(total_sale_percent + '%');
      //   $( api.column( 5 ).footer() ).html(total_vat_in);
      //   $( api.column( 6 ).footer() ).html(total_cogs);
      //   $( api.column( 7 ).footer() ).html(total_gp);
      //   $( api.column( 8 ).footer() ).html(total_gp_percent + '%');
      //   $( api.column( 9 ).footer() ).html(total_margin_percent + '%');
      ...
      footerCallback: function ( row, data, start, end, display ) {
        ...
        $.ajax({
          method:"get",
          dataType:"json",
          url:"{{ route('get-margin-report-5-footer') }}",
          data:{ from_date: $('#from_date').val(),to_date: $('#to_date').val(),product_id: $('.product_id option:selected').val(),selecting_cust_cat: $('.selecting-cust-cat').data('id'),sale_person_selected : $('.customer_selected_select option:selected').val()},
          beforeSend:function(){
          },
    ...
    table2.on( 'column-visibility.dt', function ( e, settings, column, state ) {
      ...
      $.post({
        ...
        dataType : "json",
        data : {type:'margin_report_by_customer',column_id:col},
        beforeSend: function(){
          $('#loader_modal').modal({
            backdrop: 'static',
            keyboard: false
...
$(document).on('click','.export-btn',function(){
    ...
    let filter = 'customer';
    var selecting_cust_cat = $('.selecting-cust-cat').data('id');
    selecting_cust_cat = (selecting_cust_cat != undefined) ? selecting_cust_cat : null;
    var sale_person_selected = $('.customer_selected_select option:selected').val()
    sale_person_selected = (sale_person_selected != undefined) ? sale_person_selected : null;
    if (from_date == '') {
    ...
    $.ajax({
      method:"get",
      url:"{{route('export-status-margin-report-by-customer')}}",
      data:{from_date_exp:from_date, to_date_exp:to_date, filter:filter, selecting_cust_cat:selecting_cust_cat, sale_person_selected:sale_person_selected, sortbyvalue:order, sortbyparam:column_name},
      beforeSend:function(){
  ...
  let type = 'margin_report_by_customer';
