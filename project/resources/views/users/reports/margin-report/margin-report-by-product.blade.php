...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Margin Report by Product Name</li>
      </ol>
  </div>
</div>
  ...
  <div class="col-lg-12 col-md-12 title-col">
    <div class="col-md-3">
      ...
      <div class="form-group mb-0">
        <input style="background-color:white;" type="text" class="form-control selecting-sale" name="selecting-sale" disabled readonly value="{{$sales_person->name}}" data-id="{{$sales_person->id}}">
      </div>
    </div>
  </div>
</div>
...
<div class="row mb-0">
  <div class="col-lg-12 col-md-12 title-col">
    <div class="col-md-3">
      <label>Secondary Filter</label>
      <div class="form-group mb-0">
        <input style="background-color:white;" type="text" class="form-control selecting-cat" name="selecting-cat" disabled readonly value="{{$category->title}}" data-id="{{$category->id}}">
      </div>
    </div>
  </div>
</div>
@endif
@if($customer !== null)
<div class="row mb-0 customer_selected_div">
  <div class="col-lg-12 col-md-12 title-col">
    <div class="col-md-3">
      <label>Customer</label>
        <select class="font-weight-bold form-control-lg form-control state-tags customer_selected_select" disabled="disabled">
          <option value="" disabled="">Customer</option>
          <option value="{{@$customer->id}}" selected> {{$customer->reference_name}} </option>
      ...
      <table class="table entriestable table-bordered text-center product-sales-report">
        ...
        <thead>
          <tr>
            ...
            <th class="nowrap">Default/Last <br> Supplier
            ...
            <th>Unit {{$global_terminologies['net_price']}}
                <span class="arrow_up sorting_filter_table" data-order="2" data-column_name="unit_copgs">
                  <img src="{{url('public/svg/up.svg')}}" alt="up" style="width:10px; height:10px; cursor: pointer;">
                </span>
                <span class="arrow_down sorting_filter_table" data-order="1" data-column_name="unit_copgs">
                  <img src="{{url('public/svg/down.svg')}}" alt="down" style="width:10px; height:10px; cursor: pointer;">
                </span>
              </th>
            ...
            <th>Margin
                <span class="arrow_up sorting_filter_table" data-order="2" data-column_name="margin">
                  <img src="{{url('public/svg/up.svg')}}" alt="up" style="width:10px; height:10px; cursor: pointer;">
                </span>
                <span class="arrow_down sorting_filter_table" data-order="1" data-column_name="margin">
                  <img src="{{url('public/svg/down.svg')}}" alt="down" style="width:10px; height:10px; cursor: pointer;">
                </span>
            </th>
          </tr>
...
<script type="text/javascript">
  ...
  $(function(e){
    ...
    //   processing: '<i class="fa fa-spinner fa-spin fa-3x fa-fw" style="color:#13436c;"></i><span class="sr-only">Loading...</span> '},
      ...
      info: true,
      "columnDefs": [
        { className: "dt-body-left", "targets": [ 0,1,2 ],  "width": "0%", },
        { className: "dt-body-right", "targets": [3,4,5,6,7,8,9,10],  "width": "20%"},
      ...
      ajax: {
        ...
        url:"{!! route('get-margin-report-2') !!}",
        data: function(data) {data.from_date = $('#from_date').val(),
         data.to_date = $('#to_date').val(), data.product_id = $('.product_id option:selected').val(),data.sale_id = $('.selecting-sale').data('id'),data.category_id = $('.selecting-cat').data('id'),
         data.sortbyparam = column_name,
         data.sortbyvalue = order,
         data.customer_selected = $('.customer_selected_select option:selected').val(),
      ...
      // },
        // {
        //   "render" : function(data , type , row){
        //   if(Number(row['sales'].replace(/,/gi,"")) !== 0)
        //   {
      ...
      //   },
        ...
        $.ajax({
          method:"get",
          dataType:"json",
          url:"{{ route('get-margin-report-2-footer') }}",
          data:{ from_date: $('#from_date').val(),to_date: $('#to_date').val(),product_id: $('.product_id option:selected').val(),sale_id : $('.selecting-sale').data('id'),category_id: $('.selecting-cat').data('id'),customer_selected :$('.customer_selected_select option:selected').val()},
          beforeSend:function(){
          },
          success:function(result){
            ...
            total_qty    = total_qty.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");

            $( api.column( 0 ).footer() ).html('Total');
            $( api.column( 3 ).footer() ).html(total_vat_out);
            $( api.column( 4 ).footer() ).html(total_sales);
            $( api.column( 5 ).footer() ).html(total_sale_percent + '%');
            $( api.column( 6 ).footer() ).html(total_vat_in);
            $( api.column( 8 ).footer() ).html(total_qty);
            $( api.column( 9 ).footer() ).html(total_cogs);
            $( api.column( 10 ).footer() ).html(total_gp);
            $( api.column( 11 ).footer() ).html(total_gp_percent + '%');
            $( api.column( 12 ).footer() ).html(total_margin_percent + '%');
          },
          error: function(){
    ...
    table2.on( 'column-visibility.dt', function ( e, settings, column, state ) {
      ...
      $.post({
        ...
        dataType : "json",
        data : {type:'margin_report_by_product',column_id:col},
        beforeSend: function(){
          $('#loader_modal').modal({
            backdrop: 'static',
            keyboard: false
  ...
  $('.reset-btn').on('click',function(){
    ...
    $("#loader_modal").modal('show');
     $(".state-tags").select2("", "");
     $('.customer_selected_select').val('').trigger('change');
    // $('.customer_selected_div').addClass('d-none');
    $('.product-sales-report').DataTable().ajax.reload();
  });
 ...
 $(document).on('click','.export-btn',function(){
    ...
    let category_id = $('.selecting-cat').data('id');
    let sale_id = $('.selecting-sale').data('id');
    let customer_selected = $('.customer_selected_select option:selected').val();
    category_id = category_id !== undefined ? category_id : null;
    sale_id = sale_id !== undefined ? sale_id : null;
    customer_selected = customer_selected !== undefined ? customer_selected : null;
    if (from_date == '') {
    ...
    $.ajax({
      method:"get",
      url:"{{route('export-status-margin-report-by-product-name')}}",
      data:{from_date_exp:from_date, to_date_exp:to_date, supplier_id:supplier_id, category_id:category_id, sale_id:sale_id, filter:filter, customer_selected:customer_selected, sortbyvalue:order, sortbyparam:column_name},
      beforeSend:function(){
  ...
  let type = 'margin_report_by_product_name';
