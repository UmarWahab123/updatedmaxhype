...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Margin Report by Customer Type</li>
      </ol>
  </div>
</div>
    ...
    <div class="entriesbg bg-white custompadding customborder">
      <table class="table entriestable table-bordered text-center product-sales-report">
        ...
        <thead>
          <tr>
            <th>Customer Types
              <span class="arrow_up sorting_filter_table" data-order="2" data-column_name="customer_type">
                <img src="{{url('public/svg/up.svg')}}" alt="up" style="width:10px; height:10px; cursor: pointer;">
              </span>
              <span class="arrow_down sorting_filter_table" data-order="1" data-column_name="customer_type">
  ...
  $(function(e){
    ...
    //   processing: '<i class="fa fa-spinner fa-spin fa-3x fa-fw" style="color:#13436c;"></i><span class="sr-only">Loading...</span> '},
      ...
      footerCallback: function ( row, data, start, end, display ) {
        ...
        $.ajax({
          method:"get",
          dataType:"json",
          url:"{{ route('get-margin-report-6-footer') }}",
    ...
    table2.on( 'column-visibility.dt', function ( e, settings, column, state ) {
      ...
      $.post({
        ...
        dataType : "json",
        data : {type:'margin_report_by_customer_type',column_id:col},
        beforeSend: function(){
          $('#loader_modal').modal({
            backdrop: 'static',
            keyboard: false
...
$(document).on('click','.export-btn',function(){
    ...
    $.ajax({
      method:"get",
      url:"{{route('export-status-margin-report-by-customer-type')}}",
      data:{from_date_exp:from_date, to_date_exp:to_date, filter:filter, product_id:product_id, sortbyparam:column_name, sortbyvalue:order},
      beforeSend:function(){
  ...
  let type = 'margin_report_by_customer_type';
