...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Margin Report by Office</li>
      </ol>
  </div>
</div>
    ...
    <div class="entriesbg bg-white custompadding customborder">
      <table class="table entriestable table-bordered text-center product-sales-report">
        ...
        <thead>
          <tr>
            <th>Office
              <span class="arrow_up sorting_filter_table" data-order="2" data-column_name="office">
                <img src="{{url('public/svg/up.svg')}}" alt="up" style="width:10px; height:10px; cursor: pointer;">
              </span>
              <span class="arrow_down sorting_filter_table" data-order="1" data-column_name="office">
            ...
            <th>Sales
              <span class="arrow_up sorting_filter_table" data-order="2" data-column_name="sales">
                <img src="{{url('public/svg/up.svg')}}" alt="up" style="width:10px; height:10px; cursor: pointer;">
              </span>
              <span class="arrow_down sorting_filter_table" data-order="1" data-column_name="sales">
                <img src="{{url('public/svg/down.svg')}}" alt="down" style="width:10px; height:10px; cursor: pointer;">
              </span>
            </th>
            <th>% Sales
...
$("#to_date").datepicker('setDate','today');
  ...
  table2.on( 'column-visibility.dt', function ( e, settings, column, state ) {
      ...
      $.post({
        ...
        dataType : "json",
        data : {type:'margin_report_by_office',column_id:col},
        beforeSend: function(){
          $('#loader_modal').modal({
            backdrop: 'static',
            keyboard: false
 ...
 $(document).on('click','.export-btn',function(){
    ...
    $.ajax({
      method:"get",
      url:"{{route('export-status-margin-report-by-office')}}",
      data:{from_date_exp:from_date, to_date_exp:to_date, filter:filter, sortbyparam:column_name, sortbyvalue:order},
      beforeSend:function(){
  ...
  let type = 'margin_report_by_office';
