...
<div class="col-lg-9 headings-color">
  <h4 class="font-weight-bold">{{$page_status[1]}}</h4>
</div>
...
<div class="col-lg-12" id="table-po">
  <div class="entriesbg bg-white custompadding customborder">
    <div class="alert alert-primary export-alert d-none"  role="alert">
      ...
      <div class="alert alert-success export-alert-success d-none"  role="alert">
        ...
        <a class="exp_download" href="{{ url('get-download-xslx','shipping.xlsx')}}" target="_blank" id=""><u>Click Here</u></a>
        </b>
      </div>
  ...
  <div class="col-lg-12 col-md-12 pl-0 pr-0 d-flex align-items-center mb-3">
    ...
    <div class="col-lg-2 col-md-2 delete-selected-item mb-3 d-none">
    ...
    <div class="col-lg-3 col-md-3 delete-selected-item d-none">
      <!-- <div class="add-items-to-po-div d-none"> -->
      <label class="pull-left font-weight-bold">Choose Group (to add PO's):</label>
      ...
      <select class="font-weight-bold form-control-lg form-control js-states state-tags add-pos-to-po-group" id="add-pos-to-po-group" name="add-pos-to-po-group">
        <option value="" disabled="" selected="">Choose PO Group</option>
        @if(@$po_groups->count() > 0)
          <optgroup label="Waiting Shipping Info">
          @foreach($po_groups as $po_group)
          <option value="{{ $po_group->id }}"> Group# {{$po_group->ref_id}} <span style="font-size: 10px;"> ( {{$po_group->ToWarehouse->warehouse_title}} ) </span> </option>
          @endforeach
          </optgroup>
        @endif
      </select>
    ...
    <div class="col-lg-3 col-md-3 delete-selected-item d-none">
        <label class="pull-left font-weight-bold">Choose Actio:</label>
        <select id="actions_for_selected_pos" class="form-control js-states state-tags">
        ...
        <div class="mt-3">
          <form method="post" class="group_to_po_form">
            <div class="form-group">
              ...
              <input type="hidden" name="page" class="form-control" value="waiting_shipping">
            </div>
...
<form id="export_received_into_stock_form">
  ...
  <input type="hidden" name="blade" id="blade" value="shipping">
...
$.ajax({
  ...
  var table2 = $('.table-po').DataTable({
    ...
    ajax:{
      ...
      url:"{!! route('get-po-w_s_info-data') !!}",
    ...
    $.ajax({
        ...
        success: function(data)
        {
          ...
          else if(data.success == false)
          {
            ...
            $('.delete-selected-item').addClass('d-none');
            $('.table-po').DataTable().ajax.reload();
            // swal({ html:true, title: '<h3 style="color:red;">Alert !!!</h3>', text:'<b>'+data.errorMsg+'</b>'});
  ...
  $(document).on("change",'.add-pos-to-po-group',function(){
    ...
    if(group_id > 0)
    {
      ...
      if(selected_ids == '')
      {
        swal({ html:true, title:'Alert !!!', text:'<b>Select Items First To Add In Group !!!</b>'});
        return false;
      }
      ...
      swal({
        title: "Alert!",
        text: "Are you sure you want to add this PO(s) into selected Group ?",
        type: "info",
        showCancelButton: true,
        confirmButtonClass: "btn-danger",
        confirmButtonText: "Yes!",
    ...
    if (isConfirm){
      $.ajax({
        method: "get",
        url: "{{ url('check-existing-po-groups') }}",
        // async: false,
        dataType: 'json',
        context: this,
        data: {group_id:group_id, selected_ids:selected_ids},
        beforeSend: function(){
          $('#loader_modal').modal({
              backdrop: 'static',
              keyboard: false
