...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Dispatch From Supplier</li>
      </ol>
  </div>
</div>
...
<div class="col-lg-9 headings-color">
  <h4 class="font-weight-bold">{{$page_status[2]}}</h4>
</div>
...
<div class="col-lg-12" id="table-po">
  ...
  <div class="alert alert-success export-alert-success d-none"  role="alert">
    ...
    <a class="exp_download" href="{{ url('get-download-xslx','dispatch_from_supplier.xlsx')}}" target="_blank" id=""><u>Click Here</u></a>
    </b>
  </div>
...
<form id="export_received_into_stock_form">
  ...
  <input type="hidden" name="blade" id="blade" value="dispatch_from_supplier">
...
$.ajax({
  ...
  var table2 = $('.table-po').DataTable({
    ...
    ajax:{
      ...
      url:"{!! route('get-po-d_f_supplier-data') !!}",
   ...
   e.preventDefault();
    ...
    console.log(selected_ids);
     ...
     if(selected_ids == '')
      ...
      swal({
        title: "Alert!",
        text: "Are you sure you want to Revert these PO(s) from Group(s) to Shipping?",
        type: "info",
        showCancelButton: true,
        confirmButtonClass: "btn-danger",
        confirmButtonText: "Yes!",
    ...
    if (isConfirm){
      $.ajax({
        method: "get",
        url: "{{ url('remove-from-existing-po-groups') }}",
        // async: false,
        dataType: 'json',
        context: this,
        ...
        success: function(data)
        {
          ...
          else
          {
            $('.delete-selected-item').addClass('d-none');
            $('.table-po').DataTable().ajax.reload();
            swal({ html:true, title: '<h3 style="color:red;">Alert !!!</h3>', text: '<b>'+data.errorMsg+'</b><br><br>'+data.reserved_qty_tds, type: "error"});
