...
<div class="row mb-0">
  ...
  <div class="row mb-0 headings-color mb-1">
    <div class="col-lg-8 col-md-6">
      <h3 class="maintitle text-uppercase fontbold">Purchase Order # <span id="po-no-id">{{$getPurchaseOrder->ref_id}}</span></h3>
      @if($getPurchaseOrder->status > 13 && $getPurchaseOrder->status != 40)
      @php
        $group = @$getPurchaseOrder->p_o_group->po_group;
        if(@$group->is_confirm == 0 && @$group != null)
        {
      @endphp
           <h5 class="maintitle text-uppercase fontbold">Group No# <a target="_blank" href="{{url('warehouse/warehouse-receiving-queue-detail',$group->id)}}" data-id="{{$group->id}}"><b>{{$group->ref_id}}</b></a></h5>
      @php
        }
        else
      ...
      @endphp
           <h5 class="maintitle text-uppercase fontbold">Group No#
           @if($group != null) <a target="_blank" href="{{url('warehouse/warehouse-completed-receiving-queue-detail', $group->id)}}" data-id="{{$group->id }}"><b>{{$group->ref_id}}</b></a>
           @else
           --
           @endif
            </h5>
    ...
    <div class="col-lg-4 col-md-6">
      <div class="row mx-0 align-items-center">
        <h3 class="maintitle text-uppercase fontbold col-8 px-0">Purchase From</h3>
  ...
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    <li class="fontbold" style="width: 180px;">Supply To Warehouse: <b style="color: red;">*</b></li>
    <span class="pl-4  {{$classss}}" data-spname="to_warehouse_id">
      @if($getPurchaseOrder->to_warehouse_id != null)
      {{$getPurchaseOrder->ToWarehouse->warehouse_title}}
      @else
      <div style="color: red;">To Warehouse Here</div>
      @endif
    </span>
    <select class="form-control warehouse_id select-common d-none" name="warehouse_id" id="warehouse_id_'.$item->id.'" style="width: 40%; margin-left: 25px; height: 40px;">
      <option value="" disabled="" selected="">Choose Warehouse</option>';
      @foreach ($warehouses as $value)
        @if($getPurchaseOrder->to_warehouse_id == $value->id)
          <option selected value="{{$value->id}}">{{$value->warehouse_title}}</option>
        @else
          <option value="{{$value->id}}">{{$value->warehouse_title}}</option>
        @endif
      @endforeach
    </select>
  </ul>
  @php
    $target_receive_date = $getPurchaseOrder->target_receive_date != null ? Carbon::parse($getPurchaseOrder->target_receive_date)->format('d/m/Y') : "";
  @endphp

  @if($targetShipDate['target_ship_date']==1)
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    <li class="fontbold" style="width: 180px;">@if(!array_key_exists('target_receiving_date', $global_terminologies)) Target Ship Date @else {{$global_terminologies['target_receiving_date']}} @endif:@if($targetShipDate['target_ship_date_required']==1) <b style="color: red;">*</b>@endif</li>
    <span class="pl-4 {{$classs}}" data-fieldvalue="{{$target_receive_date}}" data-spname="target_receive_date">
  ...
  @endphp
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    <li class="fontbold" style="width: 180px;">Invoice Date: <b style="color: red;">*</b></li>
  ...
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    <li class="fontbold" style="width: 180px;">Payment Terms: <b style="color: red;">*</b></li>
    <span class="pl-4 {{$classs}}" data-spname="payment_terms_id">
  ...
  @php
    ...
    {
      $exchangeClass = 'inputDoubleClick';
    }
    else
    {
      $exchangeClass = 'inputDoubleClick';
  ...
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    <li class=" fontbold" style="width: 180px;">Invoice exchange rate:</li>
    @php
    if($getPurchaseOrder->exchange_rate !== null)
    {
      $exchange = (1 / $getPurchaseOrder->exchange_rate);
    }
    else
    {
      $exchange = $getPurchaseOrder->exchange_rate;
    }
    @endphp
    <span class="pl-4 {{$exchangeClass}}" data-fieldvalue="{{@$exchange}}" data-spname="exchange_rate">
      @if($exchange != null)
      {{@$exchange}}
      @else
  ...
  @php
    $payment_due_date = $getPurchaseOrder->payment_due_date != null ? Carbon::parse($getPurchaseOrder->payment_due_date)->format('d/m/Y') : "" ;
  @endphp
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    ...
    <span class="pl-4 payment_due_date_term" data-fieldvalue="{{$payment_due_date}}" data-spname="payment_due_date">
  ...
  <ul class="d-flex mb-0 pt-2 list-unstyled">
    <li class="fontbold" style="width: 180px;">Supplier Invoice Number:</li>
    <span class="pl-4 pt-0 inputDoubleClick" data-fieldvalue="{{@$getPurchaseOrder->invoice_number}}" data-spname="invoice_number">
      @if($getPurchaseOrder->invoice_number != null)
      {{$getPurchaseOrder->invoice_number}}
      @else
      Invoice Number Here
      @endif
    </span>
    <input type="text" class="ml-4 mt-2 d-none invoice_number fieldFocus" name="invoice_number" id="invoice_number" value="{{@$getPurchaseOrder->invoice_number}}">
  </ul>
  ...
  <div class="bg-white table-responsive pb-5 custompadding">
    <div class="col-lg-12 col-md-12 mt-4">
        ...
        <div class="alert alert-success export-alert-success d-none"  role="alert">
            ...
            <a class="exp_download" href="{{ url('get-download-xslx','Waiting Conformation PO Export.xlsx')}}" target="_blank" id=""><u>Click Here</u></a>
            </b>
          </div>
      </div>
    <table class="table entriestable po-porducts-details text-center table-bordered" style="width:100%">
      <thead class="sales-coordinator-thead headings-color table-bordered">
        <tr>
          ...
          <th @if(in_array(13,$hidden_columns_by_admin)) class="noVis" @endif>
            {{$global_terminologies['quantity']}}
          ...
          <th @if(in_array(15,$hidden_columns_by_admin)) class="noVis" @endif> {{$global_terminologies['quantity_inv']}}
    ...
    <div class="row ml-4">
      <div class="col-4 mt-2 mb-4">
        <div class="purch-border input-group custom-input-group">
          @if($getPurchaseOrder->status == 12 || $getPurchaseOrder->status == 14)
    ...
    <div class="row ml-4">
      ...
      <div class="col-lg-6 mt-3">
        <a href="javascript:void(0);" style="float: right;">
          ...
          <button type="button" data-id={{$id}} class="btn-color btn purchasingSupplybtn confirm-po-btn"><i class="fa fa-check"></i> Confirm Invoice</button>
        </a>
      </div>
      @endif
...
<form id="export_waiting-con_po_form">
  @csrf
...
$('#add_notes_modal').on('hidden.bs.modal', function () {
  ...
  $('.sorting_filter_table').on('click',function(){
    ...
    $(document).on('submit', '#export_waiting-con_po_form', function (e) {
        e.preventDefault();
        var url = "{{route('export-waiting-conformation-po')}}";
    ...
    if ((e.keyCode === 13 || e.which === 0) && $(this).hasClass('active')) {
      ...
      if ($(this).attr('name') == 'quantity')
      {
        ...
        else if ($(this).val() !== '' && $(this).hasClass('active'))
        {
          ...
          $.ajax({
            type: "post",
            url: "{{ route('get_pogpd__id') }}",
            dataType: 'json',
            data: 'rowId=' + rowId + '&' + 'po_id=' + po_id,
            success: function(data) {
              pogpd__id = data.pogpd__id;
              $.ajax({
                type: "post",
                url: "{{ route('save-po-product-quantity') }}",
                dataType: 'json',
                data: 'rowId=' + rowId + '&' + 'po_id=' + po_id + '&quantity=' + attr_name + '&' + 'old_value=' + old_value + '&' + 'pogpd__id=' + pogpd__id,
                beforeSend: function() {
                  // alert('here');
                  // $('#loader_modal').modal({
                  //   backdrop: 'static',
  ...
  $(document).on('click', '.confirm-po-btn', function(e) {
    ...
    if (invoice_date == '') {
      swal({
        html: true,
        title: 'Alert !!!',
        text: '<b>Must Fill Invoice Date!!!</b>'
      });
      return false;
    }
      ...
      function(isConfirm) {
        if (isConfirm) {
          ...
          $.ajax({
            ...
            success: function(response) {
              // $('#loader_modal').modal('hide');
              if (response.success === true && response.status == "waiting-shipping-info")
              {
                toastr.success('Success!', 'Purchase Order Confirmed Successfully.', {
                  "positionClass": "toast-bottom-right"
                });
                setTimeout(function() {
              ...
              else if (response.success === true && response.status == "received-into-stock")
              {
                toastr.success('Success!', 'Purchase Order Confirmed Successfully.', {
                  "positionClass": "toast-bottom-right"
                });
              ...
              else if (response.success == false && response.status == "waiting-shipping-info")
              {
  ...
  $(document).on('click', '.add_product_to', function(e) {
    ...
    $.ajax({
      method: "post",
      data: 'selected_products=' + input + '&po_id=' + po_id + '&purchasing_vat='+ "{{in_array(17,$hidden_columns_by_admin)}}",
