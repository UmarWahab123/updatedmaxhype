        ...
        @if($result->is_retail == 'qty' && $result->quantity != 0 || $result->is_retail == 'pieces' && $result->number_of_pieces != 0)
                <tr>
                  ...
                  <td align="center" valign="top">
                    ...
                    @endphp
                    {{number_format(@$unit_price_with_vat,2,'.',',')}}
                ...
                <td>
                  ...
                  {{number_format(number_format((@$unit_price_with_vat*$num_to_multiply) - $dis_value,0,'.',''),2,'.',',')}}
                </td>
                </tr>
                @endif
                @endforeach
 	...
 	<div class="d-inline-block" style="float: right;margin-top: 0px;width: 20%;font-size: 12px;">
      ...
      @if(@$with_vat == null)
 			<tr>
 				...
 				<td align="right">{{number_format(floor(@$first_total*100)/100,'2','.',',')}}</td>
 			</tr>

 			<tr>
 				<td align="center" class="p-2"><span class="custom_font">ภาษี<br></span><span>Vat :</span> </td>
 				<td align="right">{{number_format(floor(@$vat_total*100)/100,'2','.',',')}}</td>
 			</tr>
      @endif

 			<tr>
 				<td align="center" class="pt-2" style="background-color: #ccc;"><b>Total <br>Amount</b></td>
 				<td align="right">{{number_format(number_format(@$order_total_at_end - @$order->discount,0,'.',''),2,'.',',')}}</td>
 			</tr>
 		</table>
 	</div>
 </div>
