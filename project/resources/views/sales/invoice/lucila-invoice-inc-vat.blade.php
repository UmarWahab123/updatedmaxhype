                ...
                <tr>
                  <td style="width: 50%;line-height: 0.6">
                    <p style="">
                      ...
                      <span style="font-size: 18px">{{ @$order->user->getCompany->billing_address}},<br> {{@$order->user->getCompany->getcountry->name.', '.@$order->user->getCompany->getstate->name.', '.@$order->user->getCompany->billing_zip }}
                        </span>
                    </p>
                    <p style="margin: 0px 0px 6px;font-size: 18px">
                      Phone: {{ $order->user->getCompany->billing_phone }} Fax: {{ $order->user->getCompany->billing_fax }}
                    </p>
                    <p style="font-size: 18px">Tax ID: {{$order->user->getCompany->tax_id}}</p>
                  ...
                  <td style="width: 65%;line-height: 0.6" align="right">
                    ...
                    <p style="margin: 0px 0px 6px;">
                      ...
                      <span style="font-size: 18px">
                        ...
                        {{@Auth::user()->getCompany->getcountry->thai_name, @Auth::user()->getCompany->getstate->thai_name, @Auth::user()->getCompany->billing_zip }}
                        <br>
                    ...
                    <p style="font-size: 18px">เลขประจำตัวผู้เสียภาษี: {{$order->user->getCompany->tax_id}}</p>
                    @endif
                  </td>
                  ...
                  <td style="width: 80%;padding: 0px 3px 0px 25px;background-color: #ccc;border-bottom: 3px solid #ccc;border-top: 3px solid #ccc;">
                   <div style="width: 100%">
                    <table class="table" style="width: 100%;background-color: white;min-height: 115px;font-size: 16px">
                      <tbody>
                        <tr>
                            <td width="100%" style="padding-left: 5px;line-height: 0.8;height: 65px;" valign="middle">
                              <strong>
                                  <span style="position: relative;"><span style="position: absolute;top: 10px;">ชื่อลูกค้าที่อยู่ <br>Customer Name, Address </span></span>
                              </strong>
                              <p style="margin-top: 50px;">
                                <span style="font-family: 'BoonRegular';
                                 ...
                                 font-style: normal;font-size: 10px;">
                                   ...
                                   {{@$customerAddress->billing_zip}}, {{@$customerAddress->billing_phone}}
                          ...
                          <tr >
                            <td width="100%" style="padding-left: 5px;min-height: 55px;line-height: 0.8"> <span>เลขประจำตัวผู้เสียภาษี <br>Tax ID:</span>
            ...
            <table class="table invoicetable custom_font" style="width: 100%;border-color: skyblue;border: 1px solid #ccc;">
              <thead align="center" style="background-color: #ccc;font-size: 16px !important;line-height: 0.7;">
                <tr>
                  ...
                  <th width="40%"><span>รายละเอียด</span><br>Description</th>

                  <th><span>จำนวน</span><br> QTY</th>
                  <th><span>หน่วย</span><br>Unit</th>
                  <th><span>ส่วนลด</span><br>Discount</th>
                  <th width="10%">Unit Price<br>(Inc VAT)</th>
              ...
              <tbody style="">
                ...
                @if((
                  (
                    ($order->primary_status == 2 || $order->previous_primary_status == 2 || $order->primary_status == 1)
                    &&
                    (
                      ($result->is_retail === 'qty' && ($result->quantity != 0 || $result->get_order_product_notes->count() > 0))
                      || ($result->is_retail == 'pieces' && ($result->number_of_pieces != 0 || $result->get_order_product_notes->count() > 0))
                      ))
                  ||
                  (
                    ($order->primary_status == 3 || $order->previous_primary_status == 3 ) && (($result->is_retail == 'qty' && ($result->qty_shipped != 0 || $result->get_order_product_notes->count() > 0)) || ($result->is_retail == 'pieces' && ($result->pcs_shipped != 0 || $result->get_order_product_notes->count() > 0)))
                    ))  &&
                    (!in_array($result->id, $products_array)
                    ))
                 @php $product_count1++;
               ...
               @endphp
                <tr>
                  ...
                  <td align="left">{{ @$result->short_desc }}
                    @if (@$result->get_order_product_notes->where('show_on_invoice',1)->count() > 0)
                    <br>
                    <span style="margin-left: 2px;">Note</span>
                  ...
                  <td align="center">
                    @if(@$result->discount != null && @$result->discount > 0){{@$result->discount}} %
                    <br>
                     @if($result->vat != null)
                  ...
                  <td align="right">
                    ...
                    @endphp
                    <span>
                    {{number_format(@$unit_price_with_vat)}}
                    </span>
                    </td>
                  ...
                  @endphp
                  @php @$order_total_at_end = @$order_total_at_end + round(@$result->total_price_with_vat,4); @endphp
                  <td align="right">
                ...
                <tr>
                    <td style="position: relative;" colspan="2">
                        <span style="font-size: 18px;">
                            <span style="">Account Name: {{@$bank->title != null ? $bank->title : ''}}
                                <span style="margin-left: 10px;">Account No.: {{@$bank->account_no != null ? $bank->account_no : ''}}</span>
                                <br>Bank: {{@$bank->description != null ? $bank->description : ''}}
                                <span style="margin-left: 10px;">Branch: {{@$bank->branch != null ? $bank->branch : ''}}</span>
                            </span>
                        </span>
                      </td>
                  <td>
...
<table style="max-height: 124pt;" class="custom_font">
  <tr>
    <td width="300pt" valign="top" style="margin-top: -15px; "><span style="font-size: 14px;">
        Delivered to : <br/>
      ...
      {{@$customerShippingAddress->title}}
      @endif
      {{@$customerShippingAddress->billing_address}},
      {{@$customerShippingAddress->billing_city}},
      {{@$order->customer->language == 'en' ? @$customerShippingAddress->getstate->name : (@$customerShippingAddress->getstate->thai_name !== null ? @$customerShippingAddress->getstate->thai_name : @$customerShippingAddress->getstate->name)}},
...
<td width="118pt" style="position: relative;">Due Date: <span style="position: absolute;margin-left: 10pt;">{{@$order->payment_due_date != null ? Carbon::parse(@$order->payment_due_date)->format('d/m/Y') : '--'}}</span>
</td>
    <td width="40pt" align="right" style="line-height: 0.6;"><span>รวมเงิน</span><br>Sub Total</td>
    ...
    <td width="75pt" align="center" valign="center" style="">THB {{@$order_total_at_end != 0 ? number_format(number_format(@$order_total_at_end,2,'.',''),2,'.',',') : 0}}</td>
...
<table style="max-height: 124pt;" class="custom_font">
   <tr >
    ...
    <td width="170pt" align="center"><span style="position: relative;"><span style="position: absolute;top: 4px;left: -20px;font-family: 'TH Sarabun New';
    font-weight: bold;font-style: normal;font-size: 16px;line-height: 1;">{{@$order->user->name}}</span></span></td>

    <td width="45pt"><span style="position: relative;"><span style="position: absolute;top: 9px;left: -90pt;">Phone No. : </td>
    <td width="20pt" style="position: relative;">
    <span style="position: absolute;left: -100pt;top: 10px;">
    {{@$order->user->phone_number!== null ?  @$order->user->phone_number : '--'}}
    </span></td>
    <!-- <td width="20pt" style="position: relative;"><span style="position: absolute;left: -15pt;top: 10px;"></span></td> -->
   ...
   <!--  <td width="60.4pt" align="right" style="line-height: 0.6;padding-top: 10px;visibility: hidden;"><span>ภาษีมูลค่าเพิ่ม</span><br>Vat</td>
    ...
    @if(@$order->ecommerce_order == 1)
      ...
      <!-- <td width="60.4pt" align="right" style="line-height: 0.6;padding-top: 0px;"><span></span><br>Del. Charges: </td> -->
   ...
   <tr style="border: 1px solid red;">
    <td colspan="3" width="350pt" style="text-transform: capitalize;background: #ccc;padding-top: 5px;text-align: left;" align="right" valign="middle">
       @php
            ...
            if(@$order->ecommerce_order == 1){
            $total = round(number_format((@$order_total_at_end+@$order->shipping) - @$order->discount,0,'.',''),2);
            }
            else{
            $total = number_format(@$order_total_at_end ,2,'.','');
            }
    ...
    @if(@$order->ecommerce_order == 1)
    <td width="61.2pt" align="center" valign="middle">THB {{number_format(number_format(((@$order_total_at_end + @$order->shipping) - @$order->discount),2,'.',''),2,'.',',')}}</td>
    @else
    <td width="61.2pt" align="center" valign="middle">THB {{number_format(number_format((@$order_total_at_end - @$order->discount),2,'.',''),2,'.',',')}}</td>
    @endif
  </tr>
...
<tr>
    ...
    @else
    <td width="53.2pt" height="28.8pt" valign="middle" colspan="2" class="" style="position: relative;"><span style="position: absolute;left: 0px;top: 7px">Ref.Name: &nbsp;&nbsp;&nbsp;{{@$order->customer->reference_name}}</span></td>
    @endif
     <!-- <td width="36"></td> -->
     <td colspan="4" valign="bottom"><span style="top:7px">Return Policy :<p style="font-size: 13px;margin: 0px;">Returns could be accepted at the time and day of delivery only.</p><p style="margin: 0;font-family: 'TH Sarabun New';
     font-weight: bold;
     font-style: normal;top:10pt;">การคืนสินค้าทางบริษัทจะยอมรับก็ต่อเมื่อมีการคืนในวันที่ทำการส่งสินค้าเท่านั้น</p></span>
...
<div>
  <table style="width: 100%">
    ...
    @endif
  <td width="25%"><span class="custom_font">Remarks: </span>
    </td>
