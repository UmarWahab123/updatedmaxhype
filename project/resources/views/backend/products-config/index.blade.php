...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Products Configuration</li>
      </ol>
  </div>
</div>
...
<div class="row mb-0">
    <div class="col-md-12 title-col">
        <div class="d-sm-flex justify-content-between">
            <h4 class="text-uppercase fontbold">Products Configuration <span class="h5">(Page Confiugration) </span></h4>
...
<!-- 
    <div class="col-6">
     ...
     <div class="col-4">
        ...
        <div class="" style="position: absolute;bottom: 12px;">
        <button class="btn btn-primary save-btn-quot-config">Save</button>
        </div>
        </div>
        </div>
    ...
    <div class="col-4">
        @if($getCategories)
        <div class="bg-white card">
        <h5 class="card-header">Customer Categories Configuration For Fixed Prices</h5>
        <div class="card-body">
        <div class="ml-2">
            <p style="font-style: italic;"><span style="color: red;">Note: </span>The Checked One(s) Means It Will Show On Product Listing</p>
        <ul style="list-style:none">
        @foreach($getCategories as $item)
        <li>
        <div class="form-check custom-checkbox custom-control">
        <input {{$item->show == 1 ? 'checked' : ''}} value="" class="form-check-input global-access cat-check" type="checkbox" data-id="{{$item->id}}" id="check_{{$item->id}}" />
        <label class="form-check-label" for="check_{{$item->id}}">{{$item->title}}</label>
        </div>
        </li>
        @endforeach
        </ul>
     ...
     <div class="col-4">
        @if($getCategories)
        <div class="bg-white card">
        <h5 class="card-header">Customer Categories Configuration For Suggested Prices</h5>
        <div class="card-body">
        <div class="ml-2">
            <p style="font-style: italic;"><span style="color: red;">Note: </span>The Checked One(s) Means It Will Show On Product Listing</p>
        <ul style="list-style:none">
        ...
        <li>
        <div class="form-check custom-checkbox custom-control">
        <input {{$item->suggested_price_show == 1 ? 'checked' : ''}} value="" class="form-check-input global-access cat-check-suggest" type="checkbox" data-id="{{$item->id}}" id="check_{{$item->id}}" />
        <label class="form-check-label" for="check_{{$item->id}}">{{$item->title}}</label>
        </div>
        </li>
        @endforeach
...
<script>
    ...
    // });
     ...
     $('.save-btn-quot-config').on('click', function() {
        ...
        $.ajax({
            url: "{{ route('update-products-config') }}",
            method: 'get',
            data: {
                menus: menus,
                menu_stat: menu_stat
    ...
    $(document).on('submit', '.add-setting-form', function(e){
      ...
      $.ajaxSetup({
       ...
       $.ajax({
          url: "{{ route('add-products-setting') }}",
          method: 'post',
