...
<div class="row">
  <div class="col-md-12">
    ...
    <ol class="breadcrumb" style="background-color:transparent; font-size: 20px; color: blue !important;">
        ...
        @endif
          <li class="breadcrumb-item active">Customer Detail Configuration</li>
      </ol>
  </div>
</div>
...
<div class="row mb-0">
    <div class="col-md-12 title-col">
        <div class="d-sm-flex justify-content-between">
            <h4 class="text-uppercase fontbold">Customer Detail Configuration <span class="h5">(Page Confiugration) </span></h4>
...
{{-- {{dd($customer_detail_page)}} --}}
<div class="row entriestable-row mt-2">
     <div class="col-6">
        @if($customer_detail_page)
        ...
        <div class="ml-2">
            ...
            @php
            $settings = unserialize($customer_detail_page->print_prefrences);
            @endphp
            {{-- {{dd($settings)}} --}}
          ...
          {!! Form::open(['method' => 'POST', 'class' => 'add-setting-form']) !!}
            <div class="form-group">
                <select name="title" class="form-control selectpicker">
                    ...
                    <option value="{{$value->slug}}">{{$value->standard_name}}</option>
...
<script>
    ...
    $('.save-btn').on('click', function() {
     ...
     $('.save-btn-prod-detail').on('click', function() {
        ...
        $.ajax({
            url: "{{ route('update-customer-detail-config') }}",
            method: 'get',
            data: {
                menus: menus,
                menu_stat: menu_stat
