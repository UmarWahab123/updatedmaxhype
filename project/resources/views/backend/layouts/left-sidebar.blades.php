...
<?php
  ...
  ?>
            ...
            <a class="nav-link dropdown-toggle" href="#" id="navbardrop2" data-toggle="dropdown" title="{{$parent_menu->title}}">
              <img src="{{asset("uploads/menu-icon/".$parent_menu->icon)}}" alt="" class="img-fluid"> <span>{{$parent_menu->title}}
        ...
        <?php
          ...
          foreach($child_menus as $ch) { ?>
            <a class="dropdown-item" href="@if(empty($ch->url)) {{route($ch->route)}} @else {{url($ch->url)}} @endif"> 
              <img src="{{asset("uploads/menu-icon/".$ch->icon)}}" alt="" width='20' class="img-fluid"> {{$ch->title}}
              @if($ch->slug!=null)
...
<?php
  ...
  else
  {
    ...
    ?>
    <li class="nav-item">
    <a class="nav-link" href="@if(empty($single_parent->url)) {{route($single_parent->route)}} @else {{url($single_parent->url)}} @endif" title="{{$single_parent->title}}">
